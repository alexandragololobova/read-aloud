...
const defaultEssay = {
    ...
    paragraphs: [
        ...
        { type: 'blockquote', text: 'The way to figure out what to work on is by working. If you\'re not sure what to work on, guess. But pick something and get started. You\'ll probably guess wrong, but that\'s fine. You\'ll learn something valuable about the world, about yourself, and about what you actually enjoy.' },
        { type: 'bullets', items: ['Choose work you have a natural aptitude for', 'Pick something you have a deep interest in', 'Make sure it offers scope to do great work', 'Don\'t worry too much about the third — just start'] },
        { type: 'paragraph', text: 'Curiosity is the key to all of this. If you\'re curious about something, follow it. Curiosity is not a static thing — it grows when you feed it and shrinks when you ignore it. The best way to find great work is to pay attention to what makes you curious, and then do that.' }
    ]
}
...
function Blockquote({ text, theme }) {
    return (
        ...
        >
      <span className="absolute -left-1 top-0 text-5xl leading-none select-none"
            style={{ color: theme.accent + '20' }}>&ldquo;</span>
        ...
        <ul className="space-y-2 my-5 pl-1">
            {items.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-base">
                    <span className="mt-0.5 flex-shrink-0" style={{ color: theme.accent }}>{bullet}</span>
                    <span>{item}</span>
                </li>
            ))}
        </ul>
...
export default function Reader({ title, content, sentences: propSentences, currentSentence }) {
    ...
    useEffect(() => {
        if (activeRef.current && readerRef.current) {
            ...
            if (
                elementRect.top < containerRect.top + 80 ||
                elementRect.bottom > containerRect.bottom - 80
            ) {
    ...
    return (
        <div ref={readerRef} className="max-w-[840px] mx-auto py-16 px-8">
            {/* Meta header */}
        ...
        <span className="px-3 py-1 rounded-full bg-app-card border border-app-border">
          12 min listen · imported from paulgraham.com · today
            ...
            {/* Title */}
            <h1 className="text-[2.75rem] font-semibold leading-tight mb-2 tracking-tight"
            ...
            {/* Author */}
            {!hasCustomContent && (
                <p className="text-lg text-app-muted mb-10 font-ui">{defaultEssay.author}</p>
            )}

            {/* Dynamic content: sentence-by-sentence with highlighting */}
            {hasCustomContent ? (
                <div className="space-y-2 mt-8">
                    {displaySentences.map((sentence, i) => (
                        <p
                            key={i}
                            ref={i === currentSentence ? activeRef : null}
                            className={`text-base leading-relaxed px-3 py-1.5 -mx-3 rounded-lg transition-all duration-300 ${
                                i === currentSentence ? '' : ''
                            }`}
                            style={
                                i === currentSentence
                ...
                <div className="space-y-5">
                    {defaultEssay.paragraphs.map((block, i) => {
                        ...
                        if (block.type === 'highlighted') {
                            return (
                                <p key={i} className="p-4 rounded-xl text-base leading-relaxed transition-all"
                                   style={{
                                       background: theme.followBox || 'rgba(0,0,0,0.05)',
                                       border: `1px solid ${theme.followBorder || 'transparent'}`,
                                   }}>
